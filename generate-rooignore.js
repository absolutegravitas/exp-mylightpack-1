 ...
 */
//https://github.com/Michaelzag/RooCode-Tips-Tricks/blob/main/roo-ignore/README.md
...
function parseExistingRooignore(rooignorePath) {
  try {
    ...
    const patterns = lines.map((line) => line.trim()).filter((line) => line.length > 0 && !line.startsWith("#"))

    // Check if it has a large files section
    const hasLargeFilesSection =
      content.includes("# Files over") && content.includes("tokens (LLM context overflow prevention)")
    ...
    for (const file of files) {
      ...
      if (processedCount % 100 === 0 || processedCount === files.length) {
        process.stdout.write(
          `\rAnalyzing files: ${processedCount}/${files.length} (${Math.floor((processedCount / files.length) * 100)}%)`
        )
      }
